...
**Импорт sklearn** откладывается до первого `ingest` / `explain` — uvicorn быстрее поднимает сокет (важно при жёстком таймауте healthcheck). В образе для Timeweb **`GEOATM_BIND_INTERNAL_8080=1`**: uvicorn всегда на **8080** внутри контейнера. **`HEALTHCHECK`** — exec-форма **`/usr/local/bin/python3 /app/deploy/healthcheck.py`**: GET **`/health`** и **`/`** на **127.0.0.1** и **localhost**, короткий таймаут HTTP (скрипт укладывается в **`HEALTHCHECK --timeout`**). Старт: **`ENTRYPOINT`** `deploy/start.sh` → **`/usr/local/bin/python3 -m uvicorn`**.
