...
**Импорт sklearn** откладывается до первого `ingest` / `explain` — uvicorn быстрее поднимает сокет (важно при жёстком таймауте healthcheck). В образе задан **`HEALTHCHECK`** через скрипт `deploy/healthcheck.py` (HEAD/GET на **`/`** и **`/health`**, порты из файла, `PORT` и запасной **8080**, успех при любом **2xx**). Старт приложения: `deploy/start.sh` → `python3 -m uvicorn …`.
