...
## `new_buildings.csv` (новостройки, опционально)

Файл в `GEOATM_DATA_DIR` (имя по умолчанию `new_buildings.csv`, см. `developments.csv_filename` в YAML) или путь через **`GEOATM_NEW_BUILDINGS_CSV`**.

| Колонка | Обязательность | Описание |
|---------|----------------|----------|
| `lat`, `lon` | да | WGS84 |
| `completion_date` (или `handover_date`, `keys_date`) | да | Дата сдачи, предпочтительно `YYYY-MM-DD` |
| `building_id` / `id` | нет | Идентификатор |
| `name` / `title` | нет | Название ЖК |
| `address` / `addr` | нет | Адрес |

Отдельный показатель **`delivery_score`** (0–1) и тег **`delivery_tier`** считаются только от горизонта сдачи; **не смешиваются** с `heuristic_score` / `ml_score` по H3.

Пример строк см. `new_buildings.example.csv` в корне кейса (скопируйте в `new_buildings.csv`).
